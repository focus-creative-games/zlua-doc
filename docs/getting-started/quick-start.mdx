...
`Demo.Run` 有 `int` 与 `string` 两个重载。可显式绑定；完整可运行示例见 [app.lua `test_overload_signature`](https://github.com/focus-creative-games/zlua-demo/blob/main/LuaScripts/app.lua)：
...
详见 [方法重载](/docs/guides/methods-and-overloads/)。
...
- [C# 调用 Lua 详解](/docs/guides/csharp-to-lua/) — 使用指南学习路径起点
...
| **下一篇** | [C# 调用 Lua](/docs/guides/csharp-to-lua/) |
