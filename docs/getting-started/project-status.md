...
权威规范：本仓库 **`docs/spec/`**（唯一语义标准）；冲突时以 **spec → Il2Cpp 源码 → impl** 裁决。详见 [规范总览](../spec/00-OVERVIEW)。
:::
...
| 验证 Player | Il2Cpp；语义以 [规范](../spec/00-OVERVIEW) 为准 |
| 性能对比 xLua | **Il2Cpp Player**；见 [性能对比](../compare/PERFORMANCE) |
| 查完整 API 语义 | [规范文档](../spec/00-OVERVIEW) |
...
与 Il2Cpp 共用同一套 **Lua 可见语义**（spec）；实现为 Expression Emit，目录对齐 Il2Cpp 模块。见 [Mono 实现说明](../impl/MONO)。
...
- [ ] 性能 / GC / 体积实测持续更新 [compare](../compare/)
- [ ] 规范与用户指南持续校正
- [ ] 迁移指南与社区反馈
...
- Luau
- 更多平台与设备实测
- 迁移指南完善（见 [迁移](../community/migration/)）
