...
- **C#→Lua**：标记 `[LuaInvoke]` 的 `static extern` 方法；Editor 下由 Weaver 注入桥接，Player 下为 InternalCall → native stub。
...
| 共享定义 | `ZLua.Common`：`LuaInvokeAttribute`、`LuaMarshalAsAttribute`、`LuaAliasAttribute`、`LuaAppDomain` | 同左 |
...
| `[LuaInvoke]` Weaver 规则 | [01-HOST-API.md](01-HOST-API.md) |
